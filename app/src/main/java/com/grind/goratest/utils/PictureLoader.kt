...
/**
 * This class manage loading pictures.
 * If picture file exist in cache, PictureLoader fetch file from cache dir,
 * else PictureLoader fetch file from web
 */

class PictureLoader(private val context: Context) {
    companion object {
        private const val userAgentString =
    ...
    private fun saveInfoToDb(photoId: Int, filePath: String): Long {
        ...
        val id = db.insert(SqlDbHelper.MAIN_TABLE_NAME, null, contentValues)
        db.close()
    ...
    fun getPicture(picture: Picture): Bitmap? {
        ...
        if (pictureByteArray != null) {
            ...
            saveInfoToDb(picture.id, pictureFile.absolutePath)
